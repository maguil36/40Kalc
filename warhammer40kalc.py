...
def rr_one():
	if btn_hit["text"] == "reroll once" and btn_wnd_rr["text"] == "reroll once" and btn_sv_rr["text"] == "reroll once":
		return {"rh": 1, "rw": 1, "rs":1}
	elif btn_hit["text"] == "reroll once" and btn_wnd_rr["text"] == "reroll once":
		return {"rh": 1, "rw": 1, "rs":0}
	elif btn_hit["text"] == "reroll once" and btn_sv_rr["text"] == "reroll once":
		return {"rh": 1, "rw": 0, "rs":1}
	elif btn_wnd_rr["text"] == "reroll once" and btn_sv_rr["text"] == "reroll once":
		return {"rh": 0, "rw": 1, "rs":1}
	elif btn_hit["text"] == "reroll once":
		return {"rh": 1, "rw": 0, "rs":0}
	elif btn_wnd_rr["text"] == "reroll once":
		return {"rh": 0, "rw": 1, "rs":0}
	elif btn_sv_rr["text"] == "reroll once":
		return {"rh": 0, "rw": 0, "rs":1}
	else:
		return {"rh": 0, "rw": 0, "rs":0}
##wiget commands
def incr_bs():
...
def fct_hit_exp():
	if btn_exp["text"] == 'explode off':
		btn_exp["text"] = "explode 2"
		btn_dka["text"] = "dakka off"
		btn_flm["text"] = "flame off"
		btn_blst["text"] = "blast off"
	elif btn_exp["text"] == 'explode 2':
		btn_exp["text"] = "explode 3"
	else:
...
def exp_attacks():
	if ''.join(i for i in btn_exp["text"] if ord(i) >=48 and ord(i) <= 59) == '':
		return 0
	else:
		return int(''.join(i for i in btn_exp["text"] if ord(i) >=48 and ord(i) <= 59))
def extra_damage_6():
...
title = tk.Label(master=window, text="Warhammer 40Kalculator Alpha Edition", font='Helvetica 18 bold', anchor="w")
